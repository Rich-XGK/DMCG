...
class GNN(nn.Module):
    def __init__(
        ...
        self.encoder_head = MLPwoLastAct(
            latent_size, [latent_size, 2 * latent_size], use_bn=use_bn
        )
        ...
        decoder_layers = (
            num_message_passing_steps if decoder_layers is None else decoder_layers
        )
    ...
    def forward(self, batch, sample=False):
        ...
        edge_batch = torch.repeat_interleave(
            graph_idx, num_edges, dim=0
        )  # (E, ) denoting which graph the edge belongs to

        x_embed = self.encoder_node(onehot_x)  # MLP(onehot_x), (N, 256)
        edge_attr_embed = self.encoder_edge(
            onehot_edge_attr
        )  # MLP(onehot_edge_attr), (E, 256)
        ...
        for i, layer in enumerate(self.prior_conf_gnns):
            ...
            x_1, edge_attr_1, u_1 = (
                layer(  # update node representations, global representations. equation (4), (5), (6)
                    extended_x,
                    edge_index,
                    extended_edge_attr,
                    u,
            ...
            x = (
                F.dropout(x_1, p=self.dropout, training=self.training) + x
            )  # h(l)_i = h(l-1)_i + *** equation (5)
            edge_attr = (
                F.dropout(edge_attr_1, p=self.dropout, training=self.training)
                + edge_attr
            )  #   h(l)_{ij} = h(l-1)_{ij} + *** equation (4)
            u = (
                F.dropout(u_1, p=self.dropout, training=self.training) + u
            )  # U(l)_G = U(l-1)_G + *** equation (6)
            ...
            cur_pos = self.random_augmentation(
                cur_pos, batch
            )  # random rotation, default False
        ...
        if not sample:
            ...
            if self.use_ss:
                ...
                with torch.no_grad():
                    ...
                    for i, layer in enumerate(self.encoder_gnns):
                        ...
                        edge_attr = (
                            F.dropout(
                                edge_attr_1, p=self.dropout, training=self.training
                            )
                            + edge_attr
                        )
                        u = F.dropout(u_1, p=self.dropout, training=self.training) + u
            ...
            latent_mean, latent_logstd = torch.chunk(
                latent, chunks=2, dim=-1
            )  # (G, 256), (G, 256)
            ...
            z = self.reparameterization(
                latent_mean, latent_logstd
            )  # (G, 256), z ~ N(mean, std^2)
        else:
            z = torch.randn_like(u_embed) * self.sample_beta
        z = torch.repeat_interleave(
            z, num_nodes, dim=0
        )  # (N, 256), for nodes in the same graph, they have the same z
        if self.reuse_prior:
        ...
        for i, layer in enumerate(self.decoder_gnns):
            ...
            for _ in range(cycle):
                ...
                if (
                    self.sg_pos
                ):  # To stabilize training, we stop gradient back-propagation through ρ, default False
    ...
    def extend_x_edge(self, pos, x, edge_attr, edge_index):
        ...
        length = (
            (sent_pos - received_pos).norm(dim=-1).unsqueeze(-1)
        )  # (E, 1), distance between two connected atoms
    ...
    def compute_loss(self, pos_list, extra_output, batch, args):
        ...
        if args.aux_loss > 0:
            for i in range(len(pos_list) - 1):
                loss_tmp, _ = self.alignment_loss(
                    pos,
                    pos_list[i].index_select(0, new_idx),
                    batch,
                    clamp=args.clamp_dist,
                )
                loss = loss + loss_tmp * (
                    args.aux_loss if i < len(pos_list) - args.cycle else 1.0
                )
                loss_dict[f"loss_pos_{i}"] = loss_tmp

        if args.ang_lam > 0 or args.bond_lam > 0:
            bond_loss, angle_loss = self.aux_loss(
                pos, pos_list[-1].index_select(0, new_idx), batch
            )
    ...
    def aux_loss(pos_y, pos_x, batch):
        ...
        random_tgt_index_sort = torch.sort(random_tgt_index, descending=True, dim=0)[1][
            :2
        ]
        tgt_1, tgt_2 = random_tgt_index_sort[0].unsqueeze(0), random_tgt_index_sort[
            1
        ].unsqueeze(0)
    ...
    def alignment_loss(pos_y, pos_x, batch, clamp=None):
        ...
        if clamp is None:
            loss = global_mean_pool(
                (pos_y - pos_x).norm(dim=-1, keepdim=True), batch.batch
            ).mean()
        else:
            loss = global_mean_pool(
                (pos_y - pos_x).norm(dim=-1, keepdim=True), batch.batch
    ...
    @staticmethod
    def alignment_loss_iso_onegraph(
        ...
        with torch.no_grad():
            ...
            min_q = v[:, :, 0]
            rotation = GNN.quaternion_to_rotation_matrix(min_q)
            t = pos_y_mean - torch.einsum(
                "kj,kij->ki", pos_x_mean.expand(total_iso, -1), rotation
            )
            rotation = torch.repeat_interleave(rotation, num_nodes, dim=0)
            t = torch.repeat_interleave(t, num_nodes, dim=0)
            pos_x = torch.einsum("kj,kij->ki", pos_x, rotation) + t
    ...
    def update_iso(
        pos_y, pos_x, batch
    ):  # update the isomorphism of the graph. Shape of pos_y and pos_x: (N, 3)
        with torch.no_grad():
            pre_nodes = 0
            ...
            for i in range(batch.num_graphs):  # for each graph
                ...
                ]  # all possible isomorphisms of the current graph
                if len(current_isomorphisms) == 1:
                    new_idx_x.append(current_isomorphisms[0] + pre_nodes)
                else:
                    pos_y_i = pos_y[
                        pre_nodes : pre_nodes + cur_num_nodes
                    ]  # (n, 3)， current graph's pos_y
                    pos_x_i = pos_x[
                        pre_nodes : pre_nodes + cur_num_nodes
                    ]  # (n, 3)， current graph's pos_x
                    pos_y_mean = torch.mean(
                        pos_y_i, dim=0, keepdim=True
                    )  # (1, 3)， mean of the current graph's pos_y
                    pos_x_mean = torch.mean(
                        pos_x_i, dim=0, keepdim=True
                    )  # (1, 3)， mean of the current graph's pos_x
                    pos_x_list = []

                    for iso in current_isomorphisms:
                        # change coordinates' order according to the isomorphism
                        # e.g. if iso = [1, 0, 2], then the new pos_x_i = [pos_x_i[1], pos_x_i[0], pos_x_i[2]]
                        pos_x_list.append(torch.index_select(pos_x_i, 0, iso))
                        # in the end, pos_x_list contains all possible pos_x_i, as a list with length len(current_isomorphisms) and each element is (n, 3)
                    total_iso = len(pos_x_list)  # total number of possible isomorphisms
                    pos_y_i = pos_y_i.repeat(
                        total_iso, 1
                    )  # (n * len(current_isomorphisms), 3), repeat pos_y_i for total_iso times
                    pos_x_i = torch.cat(
                        pos_x_list, dim=0
                    )  # (n * len(current_isomorphisms), 3)
                    min_idx = GNN.alignment_loss_iso_onegraph(  # find the best isomorphism according to the alignment loss
                        pos_y_i,  # (n * len(current_isomorphisms), 3)
                        pos_x_i,  # (n * len(current_isomorphisms), 3)
                        pos_y_mean,  # (1, 3)
                        pos_x_mean,  # (1, 3)
                        num_nodes=cur_num_nodes,  # n
                        total_iso=total_iso,  # len(current_isomorphisms)
                    )
                    new_idx_x.append(current_isomorphisms[min_idx.item()] + pre_nodes)
                pre_nodes += cur_num_nodes

            # new_idx_x is a list of int numbers, each number is the index of the node in the new pos_x
            # e.g. if original pos order is [0, 1, 2, 3, 4], and new pos order is [2, 0, 1, 4, 3], then new_idx_x = [2, 0, 1, 4, 3]
            res = torch.cat(new_idx_x, dim=0)
            return res
    ...
    def get_score(self, batch, pos, args):
        ...
        logq = (
            -latent_logstd
            - 0.5 * (latent_mean - z) ** 2 / (torch.exp(latent_logstd)) ** 2
        )
...
def one_hot_atoms(atoms):
    ...
    for i in range(atoms.shape[1]):
        one_hots.append(
            F.one_hot(atoms[:, i], num_classes=vocab_sizes[i])
            .to(atoms.device)
            .to(torch.float32)
        )
...
def one_hot_bonds(bonds):
    ...
    for i in range(bonds.shape[1]):
        one_hots.append(
            F.one_hot(bonds[:, i], num_classes=vocab_sizes[i])
            .to(bonds.device)
            .to(torch.float32)
        )
    return torch.cat(one_hots, dim=1)
